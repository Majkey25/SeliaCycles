...
data class MyCalendarPeriodRow(val date: Int, val periodLength: Int)
...
object MyCalendarTransformer {
    fun transform(
        ...
        periods.forEach { row ->
            ...
            if (row.periodLength !in 1..14) {
                unsupported++
            } else {
                repeat(row.periodLength) { offset ->
        ...
        return try {
            ...
            SQLiteDatabase.openDatabase(databaseFile.path, null, SQLiteDatabase.OPEN_READONLY).use { database ->
                ...
                MyCalendarTransformer.transform(
                    ...
                    periods = readPeriods(database),
                    notes = readNotes(database),
                )
            }
        } catch (error: MyCalendarFormatException) {
            throw error
            ...
            if (databaseFile.exists() && !databaseFile.delete()) databaseFile.deleteOnExit()
        }
    }
